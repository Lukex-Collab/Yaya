...
func NewChatPipeline(pool *pgxpool.Pool, rdb *redis.Client, deepseek *openai.Client) *ChatPipeline {
	return &ChatPipeline{pool: pool, rdb: rdb, deepseek: deepseek}
}
...
func (h *ChatPipeline) embedText(ctx context.Context, text string) ([]float64, error) {
	resp, err := h.deepseek.Embeddings.New(ctx, openai.EmbeddingNewParams{
