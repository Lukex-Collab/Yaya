...
func (s *Service) Create(ctx context.Context, userID, content string, isPrivate bool) (*Journal, error) {
	j := &Journal{
		...
		Title:     content[:min(30, len([]rune(content)))] + "...",
		Emotion:   "calm",
	}

	// 同步尝试 AI 分析（3秒超时），失败则用 fallback
	analyzeCtx, cancel := context.WithTimeout(ctx, 3*time.Second)
	defer cancel()
	s.analyzeJournal(analyzeCtx, j)

	_, err := s.pool.Exec(ctx,
		`INSERT INTO journals (id, user_id, title, content, emotion, is_private, word_count, created_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		j.ID, j.UserID, j.Title, j.Content, j.Emotion, j.IsPrivate, len([]rune(j.Content)), j.CreatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (s *Service) analyzeJournal(ctx context.Context, j *Journal) {
	...
	// 同步更新 in-memory（Create 返回时立即可用）
	if result.Title != "" {
		j.Title = result.Title
	}
	if result.Emotion != "" {
		j.Emotion = result.Emotion
		j.EmotionScore = result.Score
	}
	// 异步更新数据库
	go func() {
		s.pool.Exec(context.Background(),
			`UPDATE journals SET title=$1, emotion=$2, emotion_score=$3, updated_at=now() WHERE id=$4`,
			j.Title, j.Emotion, j.EmotionScore, j.ID,
		)
	}()
}

// fallbackEmotion 基于关键词的简单情绪检测（无需 AI）
func (j *Journal) fallbackEmotion() string {
	content := j.Content
	switch {
	case containsAny(content, []string{"开心", "哈哈", "高兴", "快乐", "好开心", "太棒", "yeah", "🎉", "😊"}):
		return "happy"
	case containsAny(content, []string{"难过", "想哭", "悲伤", "伤心", "哭", "😢", "😭"}):
		return "sad"
	case containsAny(content, []string{"焦虑", "紧张", "担心", "害怕", "不安", "😰"}):
		return "anxious"
	case containsAny(content, []string{"累", "困", "疲惫", "好累", "不想动", "😴"}):
		return "tired"
	case containsAny(content, []string{"兴奋", "激动", "太开心", "惊喜", "🤩"}):
		return "excited"
	default:
		return "calm"
	}
