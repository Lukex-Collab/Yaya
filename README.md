...
- **九只宠物灵魂引擎**（`ai.js`）：每只宠物有独立性格标签（勇气/好奇/社交/慵懒/话痨）、口头禅、喜好与恐惧，气泡台词与日记由性格参数驱动，可无缝切换真 LLM（设置 `window.YAYA_LLM`）
...
- **宠物模型**：9 只宠物 GLB（牙牙/貔貅/胖达/猫头鹰/龙/灵狐/鲸鱼/章鱼/熊猫），2D 版预渲染为 512×512 透明精灵图
- **SPZ 世界**：6 个真实扫描场景（约 16~31 MB 每个），由 2D 版按机位预渲染为背景图，3D 版可运行时加载
- **二维码**：9 只宠物各一张防伪激活码，配 `tools/make_qr.py` 重新生成
